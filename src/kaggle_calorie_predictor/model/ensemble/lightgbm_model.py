...
def tune_lightgbm_params(X, y, n_trials=30, n_splits=5):
    def objective(trial):
        params = {
            "n_estimators": 1000,
    ...
    log_path = Path("data/tuning_results_lightgbm.csv")
    ...
    print(f"[✓] Best LightGBM RMSLE: {study.best_value:.5f}")
...
def train_lightgbm_fold(
    ...
    params["verbosity"] = -1  # Ensure verbosity is set to -1 for silent mode
    if params is None:
        params = {
            "n_estimators": 1000,
            "learning_rate": 0.05,
